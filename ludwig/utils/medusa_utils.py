# Source: https://github.com/FasterDecoding/Medusa/blob/main/medusa/model/utils.py
...
def generate_medusa_buffers(medusa_choices, device="cuda"):
    ...
    2. medusa_attn_mask: The attention mask designed specifically for the Medusa structure, ensuring proper attention
           computation.
    3. medusa_position_ids: Denotes the position identifiers used within the Medusa structure.
    4. retrieve_indices: Provides indices that map items from a tree structure back to their original positions in a
           cartesian product.
    5. list_indices: Represents indices mapping items from a tree back to a list. This is intended for a future feature 
...
def generate_candidates(medusa_logits, logits, medusa_topk, tree_indices, temperature):
    ...
    - temperature (float): Scaling factor to modulate the logits' values before generating candidates (not used in this 
          function but kept for future extensions).
...
def tree_decoding(
    ...
    - retrieve_indices (list or torch.Tensor): Indices mapping from tree to cartesian product, used to reorder the 
          logits.
