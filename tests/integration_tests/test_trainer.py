...
from ludwig.constants import BATCH_SIZE, DEFAULT_BATCH_SIZE, EVAL_BATCH_SIZE, LEARNING_RATE, TRAINER
...
def test_tune_batch_size_and_lr(tmpdir, eval_batch_size, is_cpu):
    ...
    def check_postconditions(model):
        ...
        if is_cpu:
            assert model.config[TRAINER][BATCH_SIZE] == DEFAULT_BATCH_SIZE
